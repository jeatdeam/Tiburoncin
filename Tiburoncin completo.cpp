		...
		if (kbhit()) {
			...
			if (presa != -1) {
				...
				if (arrpeces[presa].y == 6) {
					...
					presa = -1;
					if (jugador.puntos >= 100  ) { return true; }
				}
			}
		}
...
void main() {
	...
	system("color 1a");
	int**mapa;
		mapa = new int*[FILAS];//matriz mapa
	for (int i = 0; i < FILAS; i++)
	{
	...
	*puntaje = 0;//inicializamos el puntaje en 0
	...
	if (movimiento(6, 2  , mapa, tiempo, nivel)) {//entramos al movimiento
		crearmapa(mapa);
		if (movimiento(8, 4 , mapa, tiempo, nivel)) {//entramos al movimiento
			crearmapa(mapa);
			if (movimiento(6, 6  , mapa, tiempo, nivel)) {//entramos al movimiento
				crearmapa(mapa);
				if (movimiento(10, 20  , mapa, tiempo, nivel)) {//entramos al movimiento
					//creditos
